...
本系统面向智慧农业温室场景。用户登录并完成 NLECloud 授权后，前端会每 5 秒请求一次传感器接口。后端从云平台读取温度、阈值、报警、大气压力、二氧化碳和风速等数据，将有效数据写入 SQLite 数据库，再由首页展示实时状态、历史列表、统计结果和趋势图。

系统同时集成 Kimi AI 助手。AI 可以读取最近的数据库历史记录进行连续对话，也可以把自然语言控制意图转换成受限的设备动作，例如设置温度上下限或控制执行器开关。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/sensors` | 获取传感器数据并入库 |
| GET | `/api/history` | 获取历史、统计和预警 |
| POST | `/api/device-switch` | 控制执行器 |
| POST | `/api/thresholds` | 设置上下限 |
| GET | `/api/ai-advice` | 获取 AI 环境建议 |
